...
<p style="margin-top: 10pt">Общественная организация «Центр школьного спорта Всеволожского района», именуемая в дальнейшем «Исполнитель», в лице Председателя Совета Алёшин Максим
    Александрович, действующего на основании Устава, с одной стороны, и <b>{{ $client_name }}</b>, <b>{{ $client_phone }}</b>, дата рождения <b>{{ $client_birth_date }}</b>,
...
<p><b>{{ $trainings_per_week }}</b> раза в неделю, <b>{{ $trainings_per_month }}</b> занятий в месяц по <b>{{ $training_duration }}</b> минут по дням недели: <b>{{ $schedule }}</b>
    (дни недели и время занятий могут меняться в зависимости от конкретных обстоятельств или по договоренности между Сторонами), в соответствии с условиями настоящего Договора, а
    Заказчик обязуется принять и оплатить их.</p>
...
<p>4.13 В случае, когда два и более Воспитанника одного Заказчика получают услугу тренировочного процесса осуществляемую Исполнителем, Заказчик имеет на уменьшение стоимости услуг
    ...
    действует только в тот момент, когда два и более Воспитанника занимаются одновременно. Скидка не распространяется на секцию «Плавание дошколята».</p>
