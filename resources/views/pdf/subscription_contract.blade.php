...
<p style="margin-top: 10pt">Общественная организация «Центр школьного спорта Всеволожского района», именуемая в дальнейшем «Исполнитель», в лице Председателя Совета Алёшин Максим
    Александрович, действующего на основании Устава, с одной стороны, и <b>{{ $client_name }}</b>,</p>
<p>(ТЕЛЕФОН ПРЕДСТАВИТЕЛЯ НЕСОВЕРШЕННОЛЕТНЕГО) <b>{{ $client_phone }}</b></p>
<p>именуемый в дальнейшем «Заказчик», с другой стороны, являющийся законным представителем
    несовершеннолетнего <b>{{ $ward_name }}</b>, <b>{{ $ward_birth_date }}</b> (Ф.И.О. , дата рождения), именуемого далее «Воспитанник», заключили настоящий договор (далее -
    Договор) о нижеследующем:</p>
<p>1. Предмет договора</p>
<p>1.1 Исполнитель организует тренировочный процесс для Воспитанника <b>{{ $trainings_per_week }}</b> раза в неделю <b>{{ $trainings_per_month }}</b> занятий в месяц по
    <b>{{ $training_duration }}</b>
    минут по спортивному направлению: <b>{{ $sport_kind }}</b> в соответствии с условиями настоящего Договора, а Заказчик обязуется принять и оплатить их.</p>
...
<p>4.11 В случае, когда Воспитанник является членом многодетной семьи, Заказчик имеет право на уменьшение стоимости услуг на 10% от стоимости указанной в пункте 4.1. В этом случае
    стоимость составит <b>{{ $discount_1_monthly_price }}</b> <b>({{ $discount_1_monthly_price_string }})</b> Скидка подтверждается справкой о многодетности. Фотография справки
    направляется на нашу почту kudrovo.sport@yandex.ru</p>
...
<p>5.2 Обмен отсканированными копиями договора, отчетов, актов, дополнительных соглашений производятся по следующим адресам электронной почты: электронная почта
    Заказчика: <b>{{ $client_email }}</b>, электронная почта Исполнителя: kudrovo.sport@yandex.ru</p>
    ...
    <div style="display: inline-block; vertical-align: top; width: 49%;">
        ...
        <div style="position: relative;">
            ...
            <div style="position: absolute; left: 40pt; top: -18pt; width: 100pt; height: 50pt; z-index: 5;">
                <img style="width: 100%; height: 100%;" src="data:image/png;base64,<?php echo base64_encode(file_get_contents(resource_path('views/pdf/sign.png'))); ?>" alt="stamp"/>
            </div>
            <div style="position: absolute; left: 100pt; top: -20pt; width: 120pt; height: 120pt; z-index: 5;">
                <img style="width: 100%; height: 100%;" src="data:image/png;base64,<?php echo base64_encode(file_get_contents(resource_path('views/pdf/stamp.png'))); ?>" alt="stamp"/>
            </div>
            @endisset
        </div>
    </div>
