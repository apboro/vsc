...
<p style="margin-top: 10pt">{{$header_of_contract}}, с одной стороны, и <b>{{ $client_name }}</b>, именуемый в дальнейшем «Заказчик», с другой стороны, являющийся законным представителем несовершеннолетнего
    <b>{{ $ward_name }}</b>, дата рождения <b>{{ $ward_birth_date }}</b>, именуемого далее «Воспитанник», заключили настоящий договор (далее - Договор) о нижеследующем:</p>
<p>1. Предмет договора</p>
<p>1.1 Исполнитель организует тренировочный процесс для Воспитанника {{ $trainings_per_week }}</b> раза в неделю, в течении одного календарного месяца. По длительности <b>{{ $training_duration }}</b>
...
<p>2.1.3 Предупреждать Заказчика об изменении платы за занятия и других платежей не менее чем за две недели до даты вступления в силу новых тарифов путем размещения информации
в групповых чатах WhatsApp</p>
...
<p>4.1 Стоимость услуг Исполнителя по настоящему договору, составляет <b>{{ $monthly_price }}</b> <b>({{ $monthly_price_string }})</b> НДС не облагается за 1 месяц занятий. </p>
...
<p>4.8 В случае пропуска занятий по причине болезни от 14 дней подряд, или по другим причинам, признанных Исполнителем уважительными,
    производится перерасчет оплаты за текущий или следующий месяц по единой системе перерасчетов, указанной в приложении договора №2.
    В этом случае Заказчик предъявляет администратору медицинскую справку из аккредитованного медицинского учреждения с печатью, адресом, контактным телефоном,
    ФИО врача и указанием даты болезни (не менее 14 дней подряд), или заявление с просьбой сделать перерасчёт с указанием причины по которой
    Воспитанник не посещал занятия.
</p>
<p>4.9 В случае невнесения предоплаты за текущий месяц и отсутствия Воспитанника на занятиях три недели, место за ним не сохраняется, настоящий
    Договор расторгается в одностороннем порядке со стороны Исполнителя. Новый договор заключается при наличии свободных мест.</p>
<p>
    4.10 В соответствии с п. 2 ст. 310, п. 2 ст. 424 ГК РФ, в течение срока действия настоящего Договора размер оплаты занятий может меняться в связи с
    изменением условий занятий, инфляционными процессами и индикативными показателями. Об изменении размеров платежей Заказчик предупреждается не менее,
    чем за две недели до даты их введения.
</p>
<p>4.11 В случае, когда Воспитанник является членом многодетной семьи либо иной льготной категории граждан, Заказчик имеет право на уменьшение стоимости услуг на 10% от стоимости,
    ...
    о многодетности. Копия справки предоставляется администратору</p>
...
<p>4.13 В случае, когда два и более Воспитанника одного Заказчика получают услугу тренировочного процесса осуществляемую Исполнителем, Заказчик имеет на уменьшение стоимости услуг
    ...
    действует только в тот момент, когда два и более Воспитанника занимаются одновременно</p>
...
<p>5.1 Стороны признают надлежащим подписание договора, отчетов, актов, дополнительных соглашений путем обмена отсканированными копиями по электронной почте.
    Такие документы считаются подписанными простой электронной подписью и приравниваются к документам на бумажном носителе.</p>
<p>5.2 Обмен отсканированными копиями договора, отчетов, актов, дополнительных соглашений производятся по следующим адресам электронной почты: электронная почта
    Заказчика: <b>{{ $client_email }}</b>, электронная почта Исполнителя: <b>{{ $training_base_email }}</b></p>
...
<p>6.1.2 Заказчиком по желанию, выраженному в письменном заявлении.
    При этом производится возврат 70% стоимости неиспользованных занятий, начиная с момента подачи заявления и до даты окончания оплаченного срока занятий.</p>
<p>6.1.3 По инициативе Исполнителя, если Заказчик или Воспитанник допускали нарушения, предусмотренные гражданским законодательством и настоящим Договором, договор расторгается в одностороннем порядке.
    При этом производится возврат 70% стоимости неиспользованных занятий, начиная с момента подачи заявления и до даты окончания оплаченного срока занятий.</p>
...
<p>9.6 Неотъемлемой частью настоящего договора являются приложения: Приложение №1, Приложение №2</p>
...
<div style="page-break-after: always"></div>
<h1 style="text-align: right">Приложение №2 к Договору №{{ $contract_number }}</h1>
<p style="text-align: right">от {{ $contract_date }}</p>
<table class="tariffs">
    <tbody>
    <tr>
        <th colspan="13">Единая система перерасчетов</th>
    </tr>
    <tr>
        <td>4 недели (абонемент)</td>
        <td>4000-4500</td>
        <td>4600-4900</td>
        <td>5000-5500</td>
        <td>5600-5900</td>
