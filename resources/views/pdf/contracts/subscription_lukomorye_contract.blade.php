...
<p>2.1.2. Обеспечить проживание Ребенка на период проведения тренировочных сборов с <b>{{ $service_start_date }}</b> г. по <b>{{ $service_end_date }}</b> г. на территории Спортивно-оздоровительной базы «<b>{{ $training_base_name }}</b>», по адресу: <b>{{ $training_base_address }}</b></p>
...
<p>2.1.5. Обеспечить охрану здоровья и безопасность пребывания Ребенка на тренировочных сборах в период с <b>{{ $service_start_date }}</b> г. по <b>{{ $service_end_date }}</b> г.</p>
...
<p>- авансовый платеж в размере <b>{{$advance_payment}}</b> рублей перечисляется Заказчиком на расчетный счет Исполнителя не позднее <b>{{$date_advance_payment}}</b> года</p>
<p>- оставшаяся сумма в размере <b>{{$price - $advance_payment}}</b> рублей перечисляется Заказчиком на расчетный счет Исполнителя не позднее <b>{{$date_deposit_funds}}</b> года.</p>
